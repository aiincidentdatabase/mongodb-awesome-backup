...
if [ "x${CLOUDFLARE_ACCOUNT_ID}" != "x" ]; then
  ...
  r2_list_files ${CLOUDFLARE_ACCOUNT_ID} ${CLOUDFLARE_R2_ACCESS_KEY} ${CLOUDFLARE_R2_SECRET_KEY} ${CLOUDFLARE_R2_PUBLIC_BUCKET}

  echo "There are files below in '${CLOUDFLARE_R2_PRIVATE_BUCKET}' R2 bucket:"
  r2_list_files ${CLOUDFLARE_ACCOUNT_ID} ${CLOUDFLARE_R2_ACCESS_KEY} ${CLOUDFLARE_R2_SECRET_KEY} ${CLOUDFLARE_R2_PRIVATE_BUCKET}
fi